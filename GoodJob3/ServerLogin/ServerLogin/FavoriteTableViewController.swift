...
class FavoriteTableViewController: UITableViewController {
    ...
    func downloadDataFromServer() -> Void {
        let urlString: String = "http://condi.swu.ac.kr/favorite/favoriteTable.php"
            ...
            do {
                if let jsonData = try JSONSerialization.jsonObject(with: receivedData, options:.allowFragments) as? [[String: Any]] {
                    for i in 0...jsonData.count-1 {
                        ...
                        newData.duty = jsonElement["duty"]  as! String
                        newData.term = jsonElement["term"]  as! String
    ...
    @IBAction func buttonLogout(_ sender: UIBarButtonItem) {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { action in
            let urlString: String = "http://condi.swu.ac.kr/login/logout.php"
            guard let requestURL = URL(string: urlString) else { return }
